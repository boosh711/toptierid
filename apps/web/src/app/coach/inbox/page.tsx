            ...
            return (
              <li key={t.id}>
                <Link href={`/coach/inbox/${t.id}`} className="card block hover:border-brand">
                  ...
                  {last && (
                    <p className="mt-1 truncate text-sm text-slate-600">{last.body}</p>
                  )}
                  {last && (
                    <p className="text-xs text-slate-400">{format(last.createdAt, "MMM d")}</p>
                  )}
                </Link>
              </li>
            );
          })}
          {threads.length === 0 && <p className="text-slate-500">No conversations yet</p>}
        </ul>
      </main>
    </>
  );
